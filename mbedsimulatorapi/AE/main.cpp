...
#define STATE GET_STATE()
	...
	void sim_serial_event(int mode)
	{
		...
		{
			STATE.serial->handler(STATE.serial->id, (SerialIrq)mode);
		}
	}
	...
	void serial_attach(serial_t* s, int mode)
	{
		STATE.serial = s;
		STATE.api->serial_attach(STATE.api, mode);
	}

	void serial_detach(serial_t* s)
	{
		UNREFERENCED_PARAMETER(s);
	...
	void serial_send_break(serial_t* s)
	{
		UNREFERENCED_PARAMETER(s);
